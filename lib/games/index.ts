...
export const getAllGames = async () => {
  ...
  try {
    ...
    const { data } = await axios.get(`https://www.hockey-reference.com/leagues/NHL_2021_games.html`)
    ...
    $('tr', table).each((_idx, row) => {
      let game: Game = {
        id: _idx,
        date: $('th[data-stat="date_game"]', row).text(),
        away_team: $('td[data-stat="visitor_team_name"]', row).text(),
        away_team_id: ($('td[data-stat="visitor_team_name"]', row)
          .html()
        ...
        home_team_id: ($('td[data-stat="home_team_name"]', row)
          .html()
          ?.match(/teams\/[\w]{3}\/2021.html/) || [''])[0]
          .replace(/teams\//, '')
          .replace(/\/2021.html/, ''),
